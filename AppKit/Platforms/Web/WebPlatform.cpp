...
// selected file's bytes come back through ak_web_load_file() (WebMain.cpp).
EM_JS(void, SeWebOpenFilePicker, (), {
    var el = document.getElementById('appkit-file-input');
    if (el) el.click();
});
...
bool WebPlatform::Initialize(const PlatformConfig& config)
{
    ...
    ImGui::StyleColorsDark();            // stock theme — swap for your own in a real app
...
bool WebPlatform::OpenFileDialog(std::string& outPath)
{
    (void)outPath;
#ifdef __EMSCRIPTEN__
    SeWebOpenFilePicker();   // async: bytes arrive via ak_web_load_file()
#endif
