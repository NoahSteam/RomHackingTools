...
const std::vector<App::PanelInfo>& App::PanelList()
{
    ...
    static const std::vector<PanelInfo> kList = {
        ...
        {"controller",      kControllerPanel,     &Panels::controller,      "Files & Input"},
...
void App::BuildUI(IPlatform& platform)
{
    ...
    DrawBuildDiscModal(platform); // modal; Build Disc Image options
    DrawBuildResultModal();       // modal; Build Disc Image summary
...
void App::BuildDefaultLayout(unsigned int dockspaceId)
{
    ...
    ImGui::DockBuilderDockWindow(kControllerPanel, bWatch);
...
{
    if (ImGui::Begin(kControllerPanel, nullptr, ImGuiWindowFlags_MenuBar))
    {
        if (ImGui::BeginMenuBar())
        {
...
void App::DrawPatchMenu(std::vector<TopBarCommand>& commands)
{
    ...
    {
        // Item enablement comes from the shared TopBarCommandEnabled policy (same one the native
        // menu bar reads), not from local patchCount/dataDir checks — one source of truth, no drift.
        ...
        ImGui::Separator();
        if (ImGui::MenuItem("Apply changes to disc", nullptr, false,
                            TopBarCommandEnabled(TopBarCommandType::ApplyChangesToDisc, state)))
            commands.emplace_back(TopBarCommandType::ApplyChangesToDisc);
        if (ImGui::MenuItem("Manage locations...", nullptr, false,
                            TopBarCommandEnabled(TopBarCommandType::ManageLocations, state)))
            commands.emplace_back(TopBarCommandType::ManageLocations);
        ImGui::Separator();
        if (ImGui::MenuItem("Save project...", nullptr, false,
                            TopBarCommandEnabled(TopBarCommandType::SaveProject, state)))
            commands.emplace_back(TopBarCommandType::SaveProject);
        if (ImGui::MenuItem("Open project..."))
            commands.emplace_back(TopBarCommandType::OpenProject);
        ImGui::Separator();
        const bool haveDir = TopBarCommandEnabled(TopBarCommandType::OpenBuildDiscImage, state);
        if (ImGui::MenuItem("Build Disc Image...", nullptr, false, haveDir))
            commands.emplace_back(TopBarCommandType::OpenBuildDiscImage);
        ImGui::SetItemTooltip("Rebuild the data track from the Data Directory; copy the original "
                              "audio tracks and boot info. Outputs BIN/CUE (or a data-only ISO).");
...
// The "Build Disc Image" options dialog: pick BIN/CUE (default) or ISO, then Build / Build &
...
void App::DrawBuildDiscModal(IPlatform& platform)
{
    ...
    ImGui::RadioButton("BIN/CUE  (recommended — keeps CD-audio tracks)", &mBuildFormatBinCue, 1);
    ImGui::RadioButton("ISO  (data track only)", &mBuildFormatBinCue, 0);
    ImGui::Spacing();
...
// original IP.BIN + PVD ids), audio/extra tracks copied verbatim from the source disc, into
// BIN/CUE or a data-only ISO. Optionally launch the result. Outcome shown in a modal.
void App::BuildDisc(IPlatform& platform, bool launch)
{
    auto report = [&](const std::string& msg) { mBuildResultText = msg; mShowBuildResult = true; };
    if (mDataDir.empty()) { report("Set a Data Directory first — the build packs the files under it."); return; }
    ...
    else
    {
        auto grab = [&](DiscImage& d, const IsoFs& fs) -> bool {
            if (!d.ReadBootHeader(iso.ipBin)) return false;
            if (fs.ok)
            {
                if (!fs.volumeId.empty()) iso.volumeId = fs.volumeId;
                if (!fs.systemId.empty()) iso.systemId = fs.systemId;
                iso.publisherId   = fs.publisherId;
                iso.preparerId    = fs.preparerId;
                iso.applicationId = fs.applicationId;
            }
            return true;
        };
        DiscImage src;
    ...
    if (!rom.empty()) iso.skipNames.push_back(PathBasename(rom));   // don't pack the source image

    // Output next to the Data Directory (outputs never live inside the packed tree).
    ...
    char sz[64];
    std::snprintf(sz, sizeof sz, "%.1f MB", double(res.totalBytes) / 1048576.0);
    std::string msg;
    msg += binCue ? "Format: BIN/CUE\n" : "Format: ISO (data track only)\n";
    msg += "Cue:    " + res.cuePath + "\n";
    for (const std::string& o : res.outputs)
        if (o != res.cuePath) msg += "        " + PathBasename(o) + "\n";
    msg += "Files:  " + std::to_string(res.fileCount) + "    Dirs: " + std::to_string(res.dirCount) + "\n";
    msg += "Tracks: " + std::to_string(res.trackCount);
    if (res.audioTracksCopied) msg += "  (" + std::to_string(res.audioTracksCopied) + " audio copied)";
    msg += "\nSize:   " + std::string(sz) + "\n";
    ...
    for (const std::string& w : res.warnings) msg += "Note: " + w + "\n";

    mLog.Info("Built disc image: " + res.cuePath + " (" + std::string(sz) + ")");
...
TopBarViewModel App::BuildTopBarViewModel() const
{
    ...
    // Patch feature enablement inputs — consulted by TopBarCommandEnabled for the Patch items.
    vm.patchLocationCount = static_cast<int>(mPatchLib.Count());
    vm.hasDataDir = !mDataDir.empty();
    return vm;
}
...
void App::ExecuteTopBarCommand(const TopBarCommand& command, IPlatform& platform)
{
    ...
    case TopBarCommandType::OpenBuildDiscImage: mOpenBuildDiscModal = true;   break;
#endif
...
NativeMenuState App::BuildNativeMenuState(const TopBarViewModel& s) const
{
    ...
    // ends can't drift; `s` already carries the patch-location count and Data Directory state.
    m.patchApplyEnabled = TopBarCommandEnabled(TopBarCommandType::ApplyChangesToDisc, s);
    m.patchManageEnabled = TopBarCommandEnabled(TopBarCommandType::ManageLocations, s);
    m.patchSaveEnabled = TopBarCommandEnabled(TopBarCommandType::SaveProject, s);
    m.buildDiscEnabled = TopBarCommandEnabled(TopBarCommandType::OpenBuildDiscImage, s);
    ...
    m.structureKey = BuildNativeMenuStructureKey(m);
...
{
    ToggleRenderLayer(mRenderOpts, layer);   // NM_LAYER_* -> se_render_opts field (NativeMenu.cpp)
}

void App::DispatchNativeMenuAction(const NativeMenuAction& a, std::vector<TopBarCommand>& commands)
{
    // Command-backed items map 1:1 to the TopBarCommand the toolbar emits (NativeMenu.cpp keeps
    // that table). Anything it doesn't claim is one of the view-only toggles App performs inline.
    TopBarCommand command;
    if (NativeMenuActionToCommand(a, command))
    {
