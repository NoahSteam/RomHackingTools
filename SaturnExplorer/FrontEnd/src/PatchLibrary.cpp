    ...
    std::vector<PatchOutcome>& outcomes) const
{
    ...
    os <<
        ...
        "PATCHES = [\n" << body.str() << "]\n\n"
        "def resolve(rel):\n"
        "    # Keep every write under BASE. A project carrying an absolute path or a '..'\n"
        "    # segment would otherwise write wherever it liked when the script is run.\n"
        "    path = os.path.normpath(os.path.join(BASE, *rel.split('/')))\n"
