...
#if 1
bool CreateSysSpreadsheets(const string& dialogImageDirectory, const string& duplicatesFileName, const string& sakura2Directory)
{
	...
	FindAllFilesWithinDirectory(sakura2Directory, allSakura1Files);
	...
	map<string, const SysFileExtractor*> sakuraTableFileMap;
	for (const SysFileExtractor& sakuraFile : sakuraTextFiles)
	{
	...
	{
		if (imageFileNameInfo.mPathOnly.find_first_of("SYS") != string::npos)
		{
	...
	for (map<string, vector<FileNameContainer>>::const_iterator directoryMapIter = directoryMap.begin(); directoryMapIter != directoryMap.end(); ++directoryMapIter)
	{
		...
		map<string, const SysFileExtractor*>::const_iterator sakuraFileIter = sakuraTableFileMap.find(tblFileName);
		...
		if (directoryMapIter->second.size() != sakuraFileIter->second->mLines.size())
		{
		...
		const SysFileExtractor* pSakuraFile = sakuraFileIter->second;
		int imageNumber = 0;
		for (const FileNameContainer& fileNameInfo : directoryMapIter->second)
		{
			...
			const bool bHasDialogEntry = false;
			const bool bIsLipsEntry = false;
			...
			const char* pEnglishText = "Untranslated";
