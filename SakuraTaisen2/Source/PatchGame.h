...
bool VerifyPatchingCode(const FileData& inFile, uint32 inOffset, Type inExpectedValue)
{
	const Type fileValue = SwapByteOrder<Type>(*(Type*)(inFile.GetData() + inOffset));
	if( fileValue != inExpectedValue )
	{
		printf("In SAKURA at 0x%08x, expecting 0x%04x but found 0x%04x\n", inOffset, inExpectedValue, fileValue);
	//	return false;
	}
...
bool PatchTextDrawingCode(const string& inSourceGameDirectory, const string& inPatchedDirectory)
{
	...
	#define WriteLong(address, cmd, origValue)    {if(!VerifyPatchingCode<uint32>(originalData, address, origValue)) return false; writeAddress = address; newLong = cmd;         sakuraBin.WriteData(writeAddress, (char*)&newLong, sizeof(newLong), true);}
	#define WriteCommand(address, cmd, origValue) {if(!VerifyPatchingCode<uint16>(originalData, address, origValue)) return false; writeAddress = address; newCommand = cmd;      sakuraBin.WriteData(writeAddress, (char*)&newCommand, sizeof(newCommand), true);}
	#define WriteByte(address, cmd, origValue)    {if(!VerifyPatchingCode<uint8>(originalData, address, origValue)) return false; writeAddress = address; newCommand1Byte = cmd; sakuraBin.WriteData(writeAddress, (char*)&newCommand1Byte, sizeof(newCommand1Byte), false);}
	...
	//SAKURA.BIN
	{
		...
		if( !originalData.InitializeFileData(inSourceGameDirectory + "\\SAKURA.BIN") )
		{
			return false;
		}
		...
		//Fix spacing for text drawing
		{
			...
			for (const uint32 startAddress : StartAddresses)
			{
				...
				WriteCommand(startAddress + 0, 0xea0c, 0x6a93); //mov 0x0c, r10 (ea0c)
				WriteCommand(startAddress + 2, 0x29af, 0x4a08); //mul r10, r9 (29af)
				WriteCommand(startAddress + 4, 0x0a1a, 0x4a08); //sts MACL, R10 (0a1a)
				WriteCommand(startAddress + 6, 0x7a30, 0x7a30); //add 0x30, r10 (7a30)

				//Horizontal Spacing : 06012e26 = > change this to 4100 (SHLL instead of SHLL2)
				WriteCommand(startAddress + 0x3e, 0x4100, 0x4108); //add 0x30, r10 (7a30)
			}
		}//Done fixing spacing
		...
		//Fix max characters per line
		{
			...
			{
				WriteByte(addressToModify, 0x1c, 0x0e);
			}

			//change 0x06012e44 0xE11A, change 0x06012E4A 0xE11C
			WriteCommand(0xde44, 0xe11a, 0xe10d);
			WriteCommand(0xde4a, 0xe11c, 0xe10e);

			//Change number of lines to 4 (03 but it's 0 based)
			WriteCommand(0xde52, 0xe103, 0xe102); //0x06012E53
			WriteByte(0xd7b5, 0x03, 0x02); //0x060127b5
			WriteByte(0xdb83, 0x03, 0x02); //0x06012b83 (untested)
			
			//Printing number of empty lines
			{
				//Need a bigger buffer for empty lines, so start writing from 060a50f0 instead of 060a5180 
				WriteCommand(0xde7a, 0x50f0, 0x5180);
				WriteCommand(0xdeb6, 0x50f0, 0x5180);

				//06012ece
				WriteByte(0xdecf, 0x1b, 0x0d); //num chars per line
				WriteByte(0xdeed, 0x03, 0x02); //num lines (0 based)

				//Another place for line counting 06012962
				WriteByte(0xd963, 0x03, 0x02);
			}
		}

		//Fix regular lips spacing
		{
			WriteByte(0xdff3, 0x08, 0x10); //horizontal spacing increment by 10, from 7110 to 7108 in memory at 06012ff2
			WriteByte(0xdfdf, 0x93, 0x90); //vertical spacing offset by 0x92, from 7190 to 7192 in memory at 06012fde
		}
		...
		//Fix special lips (where text changes midway) code to allow for more characters
		{
			...
			WriteCommand(0xe704, offsetToLipsCharSpriteData + 0, 0x01d0); //from 0x01d0 in memory at 06013704
			WriteCommand(0xe706, offsetToLipsCharSpriteData + 2, 0x01d2); //from 0x01d2 in memory at 06013706
			WriteCommand(0xe482, offsetToLipsCharSpriteData + 4, 0x01d4); //from 0x01d4 in memory at 06013482
			...
			WriteCommand(0xe380, 0x72ff, 0x72f0);//06013380 add 0xf0, r2 72f0  centering horizontally
			WriteByte(0xe4f5, 0x08, 0x10); //from add 0x10,r1 to add 0x08,r1
			WriteCommand(0xe45e, 0x719a, 0x7198); //from add 0x98, r1, to add 0x9a, r1 at 0601345e 
		}

		//Fix item drawing code
		{
			//Reading Code :
			WriteByte(0x4ca29, 0x1c, 0x0e);//06051a28 mov 0xe, r1 needs to be 0x1c
			WriteByte(0x4ca7d, 0x1b, 0x0d);//06051a7c mov 0xd, r1 needs to be 0x1b
			WriteByte(0x4ca83, 0x1c, 0x0e);//06051a82 mov 0xe, r1 needs to be 0x1c
			WriteByte(0x4ca8b, 0x03, 0x02);//06051a8a mov 0x2, r1 needs to be 0x3
			
			//Writing Code :
			WriteByte(0x4B641, 0x1C, 0x0e);//06050640 mov 0xe, r2 needs to be 0x1c (stride)
			WriteByte(0x4cad3, 0x1C, 0x0e);//06051ad2 mov 0xe, r1 needs to be 0x1c (stride)
			WriteByte(0x4cb07, 0x1b, 0x0d);//06051b06 mov 0xd, r10 needs to be 0x1b (char count)
			WriteByte(0x4cb25, 0x03, 0x02);//06051b24 mov 0x2, r1 needs to be 0x3 (num lines)
			
			//Buffer address :
			WriteLong(0x4cab0, 0x060a6700, 0x060a68f0);//Read:  06051ab0 needs to be 060a6700
			WriteLong(0x4caec, 0x060a6700, 0x060a68f0);//Write : 06051aec needs to be 060a6700

			WriteCommand(0x3010, 0xe56c, 0xe538); //e538 to e56c Number of bytes within text buffer to clear when opening the item menu
		}
		...
		#if USE_SINGLE_BYTE_LOOKUPS
		{
			...
			WriteCommand(0xdb08,  0x0009, 0x300c); //No-op for add r0, r0
			WriteCommand(0xdb0a,  0x011c, 0x011d); //06012b0a Text Read at 06012b0a: mov.w @(r0, r1), r1  change to 011c
			WriteCommand(0xd8dc,  0x0009, 0x300c); //No-op for add r0, r0
			WriteCommand(0xd8de,  0x011c, 0x011d); //060128de Text Read at 060128de: mov.w @(r0, r1), r1  change to 011c //when dumping out all text
			WriteCommand(0x1631a, 0x0009, 0x300c); //No-op for add r0, r0
			WriteCommand(0x1631c, 0x011c, 0x011d); //Text Read at 0601B31c: mov.w @(r0, r1), r1  change to 011c //ogami's lines
		...
		//	WriteCommand(0xd700,  0x011c); //06012700: same change to 011c
			...
			//Lips
			WriteCommand(0xdf4a, 0x0009, 0x300c);
			WriteCommand(0xdf4c, 0x014c, 0x014d); //Lips Text Read at 06012f4c: mov.w @(r0, r4), r1 (add r0, r0 right above)      change to 014c
			WriteCommand(0xdf4e, 0x611c, 0x611d); //extu.w r1, r1 06012f4e (Reading High Bits)
			WriteCommand(0xdf66, 0x00ff, 0xffff); //ffff to 00ff 06012f66 (Reading High Bits)
			...
			WriteCommand(0xe036, 0x00ff, 0xffff); //ffff to ff 06013036(Reading High Bits)
			WriteCommand(0xe25c, 0x0009, 0x300c);
			WriteCommand(0xe25e, 0x014c, 0x0014d); //Lips Text Read at 0601325e: mov.w @(r0, r4), r1   (add r0, r0 right above)    change to 014c
			WriteCommand(0xe274, 0x014c, 0x014d); //Lips Text Read at 06013274: mov.w @(r0, r4), r1                               change to 014c
			WriteCommand(0xe3dc, 0x61a0, 0x61a1); //Lips Text Read at 060133dc: mov.w @r10, r1                                    change to 61a0
			WriteCommand(0xe3f6, 0x0009, 0x300c);
			WriteCommand(0xe428, 0x02ac, 0x02ad); //Lips Text Read at 06013428: mov.w @(r0, r10), r2 (add r0, r0 060133f6)        change to 02ac
			WriteCommand(0xe4fa, 0x0009, 0x300c);
			WriteCommand(0xe4fc, 0x01ac, 0x01ad); //Lips Text Read at 060134fc: mov.w @(r0, r10), r1 (add r0, r0 right above)     change to 01ac
			...
			WriteCommand(0xd6fe, 0x011c, 0x300c);	//060126fe nop                  /011d to 011c extu.b r1, r1
			WriteCommand(0xd700, 0x611c, 0x011d);	//6012700 mov.b @(r0, r1), r1  //300c to 611c

			WriteCommand(0xd7f6, 0x011c, 0x300c);	//060127f6 add r0, r0          //300c to 011c
			WriteCommand(0xd7f8, 0x611c, 0x011d);	//060127f8 mov.w @(r0, r1), r1 //011d to 611c extu.b r1, r1
			WriteCommand(0xd810, 0x011c, 0x300c);	//06012810 add r0, r0          //300c to 0009
			WriteCommand(0xd812, 0x611c, 0x011d);	//06012812 mov.w @(r0, r1), r1 //011d to 011c  Swap with above and change this to extu.b r1, r1 (611c)
			WriteCommand(0xdd2a, 0x611c, 0x611d);	//06012d2a extu.w r1, r1       //611d to 611c
			WriteCommand(0x16316,0x601c, 0x601d);	//0601b316 extu.w r1, r0       //601d to 601c
			WriteCommand(0x1631e,0x611c, 0x611d);	//0601b31e exts.w r1, r1       //611d to 611c

			//More Lips commands along with fix for all lines flipping instead of just the newly appeared one
			WriteCommand(0xe262, 0x611c, 0x611d); //06013262  extu.w r1, r1      //611d to 611c
			WriteCommand(0xe26c, 0x015c, 0x015d); //0601326c mov.w @(r0, r5), r1 //015d
			WriteCommand(0xe26e, 0x621c, 0x621d); //0601326e extu.w r1, r2       //621d to 621c
			WriteCommand(0xe276, 0x611c, 0x611d); //06013276 extu.w r1, r1       //611d to 611c
			WriteCommand(0xe284, 0x0009, 0x300c); //06013284 add r0, r0          //300c to 0009 
			WriteCommand(0xe286, 0x014c, 0x014d); //06013286 mov.w @(r0, r4), r1 //014d  to 014c
			WriteCommand(0xe288, 0x611c, 0x611d); //06013288 extu.w r1, r1       //611d to 611c
			WriteCommand(0xe29a, 0x00ff, 0xffff); //0601329a ffff to ff
			WriteCommand(0xe3e0, 0x611c, 0x611d); //060133e0 extu.w r1, r1       //611d to 611c
			WriteCommand(0xe4ae, 0x00ff, 0xffff); //060134ae ffff to ff
			WriteCommand(0xe42e, 0x622c, 0x622d); //0601342e extu.w r2, r2       //622d to 622c			
			WriteCommand(0xe4fe, 0x611c, 0x611d); //060134fe extu.w r1, r1       //611d to 611c			
			...
			WriteCommand(0x275A4, 0x012c, 0x012d); // 0602c5a4 mov.w @(r0, r2), r1 change to 012c
			WriteCommand(0x275B2, 0x6134, 0x6135); // 0602c5b2 mov.w @r3 + , r1   6134
			WriteCommand(0x275B6, 0x6130, 0x6131); // 0602c5b6 mov.w @r3, r1    6130			
			WriteCommand(0x275B4, 0x2710, 0x2711); // 0602c5b4 mov.w r1, @r7 2710
			WriteCommand(0x275C2, 0x2710, 0x2711); // 0602c5c2 mov.w r1, @r7 2710
			WriteCommand(0x275BE, 0x7701, 0x7702); // 0602c5be add 0x2, r7 //spacing change to 7701
			WriteCommand(0x27580, 0x6170, 0x6171); // 0602c580 mov.w @r7, r1 6170
			WriteCommand(0x27582, 0x6170, 0x6171); // 0602c582 mov.w @r7, r1 6170
			WriteCommand(0x2758A, 0x7701, 0x7702); // 0602c58a add 2, r7     7701			
			WriteCommand(0x275EA, 0x00ff, 0xffff); // 0602c5ea change to 00ff
			WriteCommand(0x27584, 0x611c, 0x611d); // 0602c584 extu.w r1, r1 change to 611c
			WriteCommand(0x275B8, 0x611c, 0x611d); // 0602c5b8 611c
			WriteCommand(0x275A8, 0x611c, 0x611d); // 0602c5a8 611c
			WriteCommand(0x275B0, 0x77ff, 0x77fe); // 0602c5b0 add 0xfe, r1 change to 77ff
			...
			WriteCommand(0x4B660, 0x666c, 0x666d); // 06050660 exut.w r6, r6       //666d to 666c

			//Auto-resume commands during Chapter 10 and 9
			WriteCommand(0xdbf6, 0x00fa, 0xfffa); // 06012bf4 fffa => 00ff
			WriteCommand(0xdb1c, 0x7105, 0x7104); // 06012b1c add 4, r1 //7104 to 7105
			...
			WriteCommand(0x24f58, 0x7001, 0x7002);// 06029f58 add 0x2, r0  // add 0x1, r0 7002 to 7001
			WriteCommand(0x24f5a, 0x6200, 0x6201);// 06029f5a mov.w @r0, r2  //6201 to 6200
			WriteCommand(0x24f5c, 0x612c, 0x612d);// 06029f5c extu.w r2, r1  //612d to 612c
			WriteCommand(0x24f76, 0x00ff, 0xffff);// 06029f74 ffff => 00ff
		}
		#endif
	}
