...
#pragma pack(pop)
	...
	bool CreateFixedHeader(const vector<SakuraTextFile::SakuraStringInfo>& inInfo, const SakuraTextFile& inSakuraFile, const vector<SakuraString>& inStrings, bool bIsMesFile)
	{
		...
		mOffsetToTable       = SwapByteOrder( ((unsigned short*)inSakuraFile.mHeaderForTextEntries.pData)[0] );
...
void DumpExtractedSakuraText(const vector<SakuraTextFile>& inSakuraTextFiles, const string& outDirectory)
{
	...
	for(const SakuraTextFile& textFile : inSakuraTextFiles)
	{
		...
		{
			fprintf(pOutFile, "Len: %zi. Data: ", sakuraString.mChars.size());
		...
		for(size_t i = 0; i < textFile.mStringInfoArray.size(); ++i)
		{
			...
			const int diffFromPrev = i == 0 ? 0 : stringInfo.mNumCharactersPrecedingThisString - textFile.mStringInfoArray[i-1].mNumCharactersPrecedingThisString;
			fprintf(pOutInfoFile, "%02x %02x %i DiffFromPrev: %i\n", stringInfo.mStringId, stringInfo.mNumCharactersPrecedingThisString, stringInfo.mNumCharactersPrecedingThisString, diffFromPrev);
		}
...
#define ConditionallyAddNewLine()\
	...
	{\
		...
		{\
			printf("LIPS line is too long[D %zi].  Needs to be a max of %i characters long. %s\n", translatedLineIndex + 1, maxCharsPerLine, textLine.mFullLine.c_str());\
	...
	for(const SakuraTextFile& sakuraFile : sakuraTextFiles)
	{
		...
		if( pMatchingTranslatedFileName )
		{
			...
			{ 
				if( sakuraFile.mLines[inTranslatedLineIndex].mChars[0].mIndex != 0 )
				{
					...
					if( !bIsMESFile )
					{
						...
						{
							printf("Not enough timing data lines in %s.  Expected %i, got %zi", sakuraFile.mFileNameInfo.mFileName.c_str(), inTranslatedLineIndex, sakuraFile.mLineTimingData.size());
			...
			{	
					...
					)
				{
					...
					{
						snprintf(dirPlusLineNumberBuffer, dirPlusLineBufferSize, "%s_%zi", sakuraFile.mFileNameInfo.mNoExtension.c_str(), translatedLineIndex);
		...
		outFile.WriteData(&fixedHeader.mStringInfo[0], fixedHeader.mStringInfo.size()*sizeof(fixedHeader.mStringInfo[0]));
#endif
		//outFile.WriteData(sakuraFile.mHeaderForTextEntries.pData, sakuraFile.mHeaderForTextEntries.dataSize);
...
void OutputDialogOrder(const string& rootSakuraTaisenDirectory, const string& outputDirectory)
{
	...
	FindAllFilesWithinDirectory(rootSakuraTaisenDirectory + "\\SAKURA1\\", allSakura1Files);
	...
	for (const SakuraTextFile& sakuraFile : sakuraTextFiles)
	{
		const string outFileName = outputDirectory + sakuraFile.mFileNameInfo.mNoExtension + extension;
		TextFileWriter writer;
		if (writer.OpenFileForWrite(outFileName))
		{
			for (size_t i = 0; i < sakuraFile.mSequenceEntries.size(); ++i)
			{
				sprintf_s(buffer, 1024, "%04x %04x\n", sakuraFile.mSequenceEntries[i].mTextIndex, sakuraFile.mSequenceEntries[i].mImageId);
...
{
	FaceWinAllExtractor extractor;

	extractor.ExtractFaceWinFiles(inRootSakuraDirectory, inOutputDirectory);
}
	...
	{	
		...
		for(unsigned int i = 0; i < faceFileHeader.mNumEntries; ++i)
		{
				...
				FileData::IsDataTheSame(uncompressedImage.mpUncompressedData, origObstacleImage.mpUncompressedData, uncompressedImage.mUncompressedDataSize) )
			{
				...
				{
					printf("Patched obstacle image data is too big when compressed. Needs to be <= %zi.  Is %i \n", uncompressedImage.mCompressedSize, patchedObstacleData.mDataSize);
					return false;
				}
...
int main(int argc, char *argv[])
{
	...
	else if( command == "ExtractFaceWinFiles" && argc == 4 )
	{
		const string rootSakuraDir = string(argv[2]) + Seperators;
		const string outDir        = string(argv[3]) + Seperators;

		ExtractFaceWinFiles(rootSakuraDir, outDir);
	}
